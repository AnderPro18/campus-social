...
export class ChatService {
  ...
  async sendMessage(chatId: string, payload: Partial<Message>): Promise<void> {
    ...
    try {
      const chatSnap = await getDoc(doc(this.firestore, `chats/${chatId}`));
      if (chatSnap.exists()) {
        const chatData = chatSnap.data() as any;
        const members: string[] = chatData.members || [];
        const me = this.auth.currentUser;
        const myDisplayName = (me as any)?.displayName || 'Usuario';
        const myAvatar      = (me as any)?.photoURL    || '';
        for (const memberId of members) {
          if (memberId !== myUid) {
            await this.ns.notifyMessage({
              toUid:      memberId,
              fromUid:    myUid,
    ...
    } catch (e) { console.warn('Notificación de mensaje fallida:', e); }
  }
